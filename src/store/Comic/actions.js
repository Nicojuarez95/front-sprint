...
const read_events = createAsyncThunk(
  ...
  try {
    let response = await axios.get(
      "https://minga-vrxh.onrender.com/mangas-form/view?title="+inputText.trim()+"&category="+captureChecks+"&page="+pages,
    );
...
const read_manga = createAsyncThunk(
  'read_manga',
  async ({ id }) => {
      let url = 'https://minga-vrxh.onrender.com/mangas-form/' + id;
