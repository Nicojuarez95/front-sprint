...
const read_events = createAsyncThunk(
  ...
  try {
    let response = await axios.get(
      "https://minga-host.onrender.com/mangas-form/view?title="+inputText.trim()+"&category="+captureChecks+"&page="+pages,
    );
...
const read_manga = createAsyncThunk(
  'read_manga',
  async ({ id }) => {
      let url = 'https://minga-host.onrender.com/mangas-form/' + id;
...
const read_chapters = createAsyncThunk(
  'read_chapters',
  async ({ id, page }) => {
      let url = 'https://minga-host.onrender.com/chapters?manga_id='+id+'&page='+ page;
